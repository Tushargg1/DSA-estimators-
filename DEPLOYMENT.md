...
## GitHub solution export

Register a [GitHub App](https://docs.github.com/en/apps/creating-github-apps/registering-a-github-app/registering-a-github-app) for repository exports. Configure its **Setup URL** as the deployed frontend origin; GitHub redirects installations there with `installation_id` and the one-time state generated by this application. Under repository permissions, grant **Contents: Read and write** and leave Metadata at its required read-only level. No webhook is required for this MVP. GitHub Apps start without permissions, so grant only these repository permissions.

Generate a private key for the App and convert it to unencrypted PKCS#8 DER before base64 encoding it:

```powershell
openssl pkcs8 -topk8 -nocrypt -in github-app.pem -outform DER -out github-app.pk8
[Convert]::ToBase64String([IO.File]::ReadAllBytes("github-app.pk8"))
```

Set the resulting values on the backend host and redeploy:

```text
GITHUB_APP_ID=<app-id>
GITHUB_APP_SLUG=<app-slug>
GITHUB_APP_PRIVATE_KEY_BASE64=<single-line-pkcs8-base64>
```

GitHub installation access tokens are minted server-side and are short-lived; the browser never receives a GitHub credential. After deployment, users open **GitHub** in the application, install the App for selected repositories, choose a destination, and generate a one-time extension token. Load `extension/` as an unpacked Chrome/Edge extension, then paste the deployed API base URL and token into its popup.

The extension captures source only after a submit action followed by an accepted result. The backend keeps the first accepted capture for each platform/problem, writes a Markdown file under `<pattern>/<problem>/<platform>-<capture-id>.md`, retries transient GitHub failures, and clears source from the database after export. See GitHub's documentation for [setup URL behavior](https://docs.github.com/en/apps/creating-github-apps/registering-a-github-app/about-the-setup-url) and [installation authentication](https://docs.github.com/en/apps/creating-github-apps/authenticating-with-a-github-app/generating-an-installation-access-token-for-a-github-app).
